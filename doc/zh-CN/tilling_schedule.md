...
### 硬件矩阵降低 (Hardware Matrix Lowering)
...
我们将这种二维矩阵称为**硬件矩阵 (Hardware Matrix)**，因为它与具体硬件相关，携带了硬件信息：
- 第一维是处理时间维度
- 第二维是空间维度

硬件矩阵的空间维度对应**硬件向量 (Hardware Vector)**，反映了硬件并行度和 tile size 的粒度。

相应地，变换之前的多维 Tensor 称为**软件张量 (Software Tensor)**。

软件张量到硬件矩阵的变换过程定义了一种**调度 (Scheduling)**，也称为**降低 (Lowering)**。

反之，硬件矩阵到软件张量的逆变换过程称为**逆调度 (De-scheduling)** 或**提升 (Raising)**，遵循 desort-detilling-depad 的逆序过程。


### 索引计算

具体到硬件实现，硬件矩阵的空间维度代表硬件计算并行度，时间维度反映控制流。控制流本质上是一种映射关系，描述了将多维索引映射到一维标量的方法。这里定义两种映射：**序列化**和**地址化**。
